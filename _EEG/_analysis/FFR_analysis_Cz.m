...
cd('_EEG/_preprocdata_FFR/Cz');
subdir = dir('UH*');
  
%%
for s=setdiff(19:length(subdir),[2])
    ...
    %chan oi
    if stimear ==1
    chans =[find(strcmp(data.label,'P9'))];%  find(strcmp(data.label,'EXG2'))]; % right tiptrode
    elseif stimear ==2
    chans =[find(strcmp(data.label,'P10'))];%  find(strcmp(data.label,'EXG1'))];
    end
...
%for s=1:length(subdir)
    ...
    fmod = 2;
    f_tone = 326;
    t=0:1/fs_aud:stimL-1/fs_aud;
    tones = sin(2*pi*f_tone*t);
    % add modulation
    tonemod = zeros(1,stimL*fs_aud);
    ons = 1:fs_aud/fmod:stimL*fs_aud+fs_aud/fmod;
    Lr = .01*fs_aud; % ramp length
    r = [sin(linspace(0, pi/2, Lr)) ones(1,fs_aud/fmod/2-Lr*2) sin(linspace(pi/2, 0, Lr))]'; % ramp

    for ii = 1:length(ons)-1
        tonemod(ons(ii):ons(ii)+fs_aud/fmod/2-1) = r;
    end
    stim_tones = tones.*tonemod;    
    % nHL compensation
    %[gainHL,f_filt] = nHLfilter(85,20:12000);
    %filtercoefs = cvt_design_IG_filter(gainHL,20:12000, fs_aud);
    stim_res = stim_tones;%filtfilt(filtercoefs,1,stim);
    % resample
    stim_res = resample(stim_res,fs,fs_aud);
    stim_res = stim_res/(max(abs(stim_res)))*0.5;
    stim_res_nocomp =resample(stim_tones,fs,fs_aud);
    % cross-correlation
    [acor,lag] = xcorr(squeeze(data_filt(s,:,:)),stim_res,'coeff');
    [c_sub(s,kk),I] = max(abs(acor))
    delay_sub(s,kk) = lag(I)
    ...
    data_ffr.tidx      = tidx;
    data_ffr.fs        = fs;
    data_ffr.xcorr.acor= acor;
    data_ffr.xcorr.lag = lag;
    data_ffr.delay     = delay_sub(s,kk);
    ...
    cd('_EEG/_FFR_results/Cz')
    %cd(subdir(s).name)
    savefile = [subdir(s).name(1:4) '_FFR_processed.mat'];
