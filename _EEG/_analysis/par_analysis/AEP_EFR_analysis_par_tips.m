...
function AEP_EFR_analysis_par_tips(s,subdir,datadir,rootdir,clin_dir)
try
...
%get fft noise
[f_noise,fft_sub_noise,~,~,~,~,~]=get_fft(noise_filt(:,1:length(data_filt)),foi,fs);
    ...
    cd(rootdir)
    cd('/work1/jonmarc/UHEAL_master/UHEAL/_EEG/_derivatives/_EFR_results')
    %cd(subdir(s).name)
    savefile = [subdir(s).name(1:5) '_EFR_processed.mat'];
...
for cc=1:chans  
    ...
    %get noise
    noisebw = 20;
    nonfreqs = [fid+[2:2:10] fid-[2:2:10]]';%[(fid+4) (fid-4)]';
    linenoise = [100]';
    nbins = find([f>=(fid-noisebw) & ...
        ...
        & f~=nonfreqs(9) & f~=nonfreqs(10)...
                & f~=linenoise]);
%        & f~=nonfreqs(11) & f~=nonfreqs(12)...
%        & f~=nonfreqs(13) & f~=nonfreqs(14)...      
