...
function FFR_analysis_par_tips_phase(s,subdir,datadir,rootdir,clin_dir)
try
    ...
    for kk=1 % condition loop
        ...
        for dd = 1:2
            ...
            foi = [326];
            time = 0:1/fs:length(data_filt_d{dd}(1,:))/fs-1/fs;
    ...
    cd(rootdir)
    cd('/work1/jonmarc/UHEAL_master/UHEAL/_EEG/_derivatives/_FFR_results/phase_tips')
    %cd(subdir(s).name)
