...
function midlate_analysis_par(s,subdir,datadir,rootdir,clin_dir)
try
rates = [9,40];
%for s=1:length(subdir)
    ...
    % go to EEG folder
    cd(rootdir)
    cd('_EEG/_preprocdata_ABR/mid_latencies')
    ...
    chans =[1:16];%find(strcmp(data.label,'Cz'))]; % right tiptrode
    ...
    for kk=[ccc] % condition loop
        ...
        tidx = find(time>=-10e-3 & time<110e-3); % chirps -1 to 15 ms
        
        epoched_data =epoch_data_3(data_cond,1,trials_oi); %epoch x chan x time
        if s==20
            epoched_data(:,16,:) = nan(size(epoched_data(:,1,:)));
        end
...
%         data_trials = catdata./epoch_var';
    ...
    cd(rootdir)
    cd('_EEG/_derivatives/_midlate_results')
    %cd(subdir(s).name)
    savefile = [subdir(s).name(1:5) '_midlate_processed.mat'];
