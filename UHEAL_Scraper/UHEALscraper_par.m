...
function UHEALscraper_par(datafol_remote) 
...
for i = 1:size(sfol,1)%1:size(sfol,1) %loop across subject/session folders
    ...
    elseif size(contX,1)+size(contM,1)+size(contB,1)size(contXl,1)+size(contD,1) < 11
        ...
        errors = [errors;[sfol(i).name, ' has fewer than the expected 11 various data files']];
    ...
    if strcmp(dataalm.id,'UH096')
        dataalm.acalos = [];
    else
     if size(contD,1) > 2
                    warning('More than 2 dat data files found in %s, extras are ignored.', sfol(i).name)
                    
                    errors = [errors;[sfol(i).name, ' has more than 2 .dat files in it.']];
     ...
     elseif isempty(contD)
         ...
         errors = [errors;[sfol(i).name, ' has no dat files in it.']];
     end  
    ...
    [~,HV]      = xlsread(eeglab_dir.name,1,'D1:D200');
    
    id = find(strcmp(dataalm.id,tmp));
    
    % get lab
    if id
        dataalm.subinfo.lab = lab_tmp{id};
        % get CP
        if strcmp(CP{id},'NH')
            dataalm.subinfo.CP  = 0;
        elseif strcmp(CP{id},'HI')
            dataalm.subinfo.CP = 1;
        end
        
        % get been to hvidovre
        if strcmp(HV{id},'Y')
            dataalm.subinfo.HV = 1;
        else
            dataalm.subinfo.HV = 0;
        end
    else %no id
        dataalm.subinfo.lab = nan;
        dataalm.subinfo.CP = nan;
        dataalm.subinfo.HV = nan;
    end
