...
%% Preamble
errors = {}; %clues to things that went wrong will be stored here

%% Detect folders and check content

% %insert here where the data is stored (manual override)
datafol = 'O:\Public\Hearing-Systems-group\cahr\Temporary_ftp\UHEAL\UHEAL_data'; %changed from 'C:\Users\sadaw\Documents\UHEAL\Data';
...
for i = 106%:size(sfol,1) %loop across subject/session folders
    cd(sfol(i).name)%enter each folder
    fprintf('Entered folder: %s \n',sfol(i).name)
    
    addpath(root); %add the parent folder to path to ensure access to req. functions
    ...
    if size(contX,1)+size(contM,1)+size(contB,1)+size(contXl,1) == 0 %% added +size(contXl,1
        warning('Folder: %s appears to have no data files in it. Skipping it!',sfol(i).name)
        errors = [errors;['Folder "' sfol(i).name, '" was empty of recognised data files']];
        cd('..') %leave the folder
        continue %skip to next folder iteration
        
    elseif size(contX,1)+size(contM,1)+size(contB,1)+size(contXl,1) < 11 % <9 is changed to <11 as we add SSQ12 and NESI files
        warning('Some files might be missing from folder %s', sfol(i).name)
        errors = [errors;[sfol(i).name, ' has fewer than the expected 9 various data files']];
        
    %checks are not performed here for whether the ACLOS files are available as the export from the BEAR GUI is too variable   
    end
    ...
    if size(contX,1) > 2
       ...
       errors = [errors;[sfol(i).name, ' has more than 2 .xml files in it.']];
    end        
        
    if isempty(contX) %if NO .xml files exist
        warning('No .xml exist in folder: %s \n', sfol(i).name)
        errors = [errors;[sfol(i).name, ' has no .xml files in it.']];
    end
    ...
    if ~isempty(contX) %if some .xml files exist
        ...
        for j = 1:size(contX,1)
            tmp = xml2struct(contX(j).name); %load in the first .xml file
            ...
            switch xmltype
                case 'AUD'
                    if checkAUD == 1
                        ...
                        errors = [errors;[sfol(i).name, ' has more than one audiogram file in it.']];
                    ...
                    %struct
                    dataalm.id = sfol(i).name;
                    dataalm.per = tmp.SaData.ClientInfo; %(creationDate field doesnt seems to work)
                    dataalm.per.AUDdate = tmp.SaData.Session.Created.Text; % add date and time of audiogram
                    dataalm.per.PersonNumber = sfol(i).name; %fill in unused struct field with folder name to record which UHEAL participant number the data is for
                    checkPER = 1;
                    ...
                    for n = 1:size(errorAUD,1)
                        errors = [errors;[contX(j).name ,' in ' ,sfol(i).name, errorAUD{n,1}]];                    
                    end
                ...
                case 'TEOAE'
                    if checkTEOAE == 1
                        ...
                        errors = [errors;[sfol(i).name, ' has more than one TEOAE file in it.']];
                    ...
                    if any([squeeze(any(any(isnan(dataalm.teoae{1}))))',any(isnan(dataalm.teoae{2})),any(isnan(dataalm.teoae{3}))])
                        warning('Some TEOAE  data is missing from file "%s" in folder %s.\n', contX(j).name, sfol(i).name)
                        errors = [errors;[contX(j).name,' in ',sfol(i).name, ' is missing data.']];
                    end
                ...
                otherwise
                    ...
                    errors = [errors;[contX(j).name, ' in folder ', sfol(i).name, 'was skipped.']];
    ...
    if size(contXl,1) > 2
       ...
       errors = [errors;[sfol(i).name, ' has more than 2 .xlsx files in it.']];
    end        
        
    if isempty(contXl) %if NO .xlsx files exist
        warning('No .xlsx exist in folder: %s \n', sfol(i).name)
        errors = [errors;[sfol(i).name, ' has no .xlsx files in it.']];
    end
        ...
        for j = 1:size(contXl,1)
            ...
            switch contXl(j).name(1:2)
                case 'NE'
                    ...
                    tmp = xlsread(contXl(j).name,1,'C38:C39'); % load/read the xlsx file
    ...
    fprintf('Processing .bdf files.\nFound %d: \n', size(contB,1))
     ...
     if size(contB,1) > 1
                    ...
                    errors = [errors;[sfol(i).name, ' has more than one EEG file in it.']];
                    
     elseif isempty(contB)
         warning('No EEG files found!')
         errors = [errors;[sfol(i).name, ' has no EEG file in it.']];
     end  
    ...
    fprintf('Processing .mat files.\nFound %d: \n', size(contM,1))
     ...
     if size(contM,1) > 7
                    warning('More than 7 Matlab data files found in %s, extras are ignored.', sfol(i).name)
                    warning('MEMR files properly named ending as "_L" or "_R" will take precidence.')
                    errors = [errors;[sfol(i).name, ' has more than 7 .mat files in it.']];
     ...
     elseif isempty(contM)
         ...
         errors = [errors;[sfol(i).name, ' has no matlab files in it.']];
     end  
    
     if ~isempty(contM) %if some .mat files exist
        
          for j = 1:size(contM,1)
               tmp = load(contM(j).name); %load in the first .mat file
               ...
               fieldsACALOS = sort({'Listener';'ID';'Location';'Examiner';'CalFile';});
               fieldsMEMR = {'data'};
               fieldsRDS = {'score'};
               ...
               if isequal(fields, fieldsACALOS)
                   %found to be an ACALOS data .mat
                   fprintf('Found an ACALOS data file.\n')
                   if checkACALOS == 1
                        warning('An ACALOS file was already read in from this folder. Additional ones are ignored.')
                        errors = [errors;[sfol(i).name, ' has more than one ACALOS file in it. Extras are ignored.']];
                        
                   elseif checkACALOS == 0
                       %copy in data
                       fprintf('Saved\n')
                       dataalm.acalos = tmp; %copy acalos data to storage struct
                       checkACALOS = 1;
                   end 
                   
               
               elseif isequal(fields, fieldsMEMR)
                   ...
                   elseif ~checkMEMRL || ~checkMEMRR %in the case the naming convention was not followed, and either a confirmed L/R MEMR file is not found yet
                       ...
                       dataalm.memr.unidentified = [dataalm.memr.unidentified,tmp.data];
                        warning('%s in %s was not named identifying if it was L/R. Saved under field "unidentified".', contM(j).name, sfol(i).name) 
                        errors = [errors;[contM(j).name,' in ',sfol(i).name, ' was not identified as L/R. Saved simply in a numbered cell under field "unidentified".\n']];
                   elseif checkMEMRL && checkMEMRR %If a confirmed both L and R MEMR files were already read in and saved.
                        warning('%s in %s was not saved and L & R files were already found.', contM(j).name, sfol(i).name) 
                        errors = [errors;[contM(j).name,' in ',sfol(i).name, ' was not saved as named L & R files were already found.']];
                   end                 
               ...
               elseif isequal(fields, fieldsRDS)
                   ...
                   if checkRDS == 1
                        ...
                        errors = [errors;[sfol(i).name, ' has more than one RDS file in it. Extras are ignored.']];
               ...
               elseif isequal(fields, {'stim'}) %see if it's a stim.mat file
                ...
                if isequal(fields, fieldsSTIMaep)
                   ...
                   if checkSTIMaep == 1
                        ...
                        errors = [errors;[sfol(i).name, ' has more than one STIM aep file in it. Extras are ignored.']];
                ...
                elseif isequal(fields, fieldsSTIMabr)
                   ...
                   if checkSTIMabr == 1
                        ...
                        errors = [errors;[sfol(i).name, ' has more than one STIM abr file in it. Extras are ignored.']];
                ...
                elseif isequal(fields, fieldsSTIMffr)
                   ...
                   if checkSTIMffr == 1
                        ...
                        errors = [errors;[sfol(i).name, ' has more than one STIM ffr file in it. Extras are ignored.']];
                   ...
                   warning('%s in %s was not identified as either a ACALOS, MEMR, RDS, or STIM data file based on contained field names. Skipped!', contM(j).name, sfol(i).name) 
                   errors = [errors;[contM(j).name,' in ',sfol(i).name, ' was not identified as either a ACALOS, MEMR, RDS, or STIM data file. It was skipped.']];
    ...
    checks = ~[checkPER,checkAUD,checkTEOAE,[checkMEMRL && checkMEMRR],checkACALOS,checkEEG,checkRDS,checkNESI,checkSSQ];
    fnames = fieldnames(dataalm);
    fnames = fnames(logical(checks));
    if ~isempty(fnames)
        fprintf(2,'Warning: Valid %s data is missing from folder %s. None was accepted and saved.\n\n',strjoin(fnames,' + '),sfol(i).name)
        errors = [errors;[ 'No valid ',strjoin(fnames,' + ') ,' data was accepted and saved from ',sfol(i).name]];
    end  
    
end
%save error file
%writecell(errors,'errorlog')
save('errorlog.mat', 'errors');
cd('..')
