...
for s=1:length(d)
    ...
    PTA_LF_new(s) = nanmean(results{s}.aud(1:4));
...
uheal_data.PTA_lf_new = PTA_LF_new';
...
saveas(fig,'figs/figs_NH/audiogram_all','epsc')
%% only NH
close all
...
saveas(fig,'figs/figs_NH/audiograms_NH','epsc')
...
saveas(fig,'figs/figs_NH/audiograms_HI','epsc')
...
%title(['all, n=' num2str(length(idx))])
fig= gcf
%saveas(fig,'figs/figs_NH/age_hist_all','epsc')
...
%title(['all, n=' num2str(length(idx))])
fig= gcf
saveas(fig,'figs/figs_NH/age_hist_male_female','epsc')
...
fig= gcf
saveas(fig,'figs/figs_NH/memr_summary','epsc')
%% MEMR slope
...
for s=1:length(d)
    ...
    for kk=1:5
        ...
        %teoae_sig(s,kk) = teoae_sub_amp(s,kk,3)
...
box off
fig= gcf
saveas(fig,'figs/figs_NH/teoae_overview_amp','epsc')
